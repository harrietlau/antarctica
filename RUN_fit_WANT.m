%% BAYESIAN FIT TO AMUNDESEN SEA EMBAYMENT VALUES
%  HL Sept 2021
%  Fit Amundsen Sea Embayment to thermodynamic state.
...
outdir='WANT/OUT_xfit_premelt_50km/';
q_method='xfit_premelt';
% stick with 50 km for this run
% these inputs were averaged external to this code
filenames.Vs = 'WANT/IN/WANT_Lloyd_Vs_50km.mat';
filenames.LAB = 'WANT/IN/WANT_LAB_50km.mat';

location.lat = -74.833; %lat; % degrees North
location.lon = 360.-102.667 ; %lon; % degrees East (0-360)
location.z_min = 200; % averaging min depth for asth.
location.z_max = 250; % averaging max depth for asth.
location.smooth_rad = 5;

fobs_LAB = 1.0; % An - model for LAB
