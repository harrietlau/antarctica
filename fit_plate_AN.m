function posterior = fit_plate_AN(filenames, location, q_method, prior_S,fobs,outdir)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% fit_plate - with Harriet & Ben definition of zlab
%
...
Files.SV_Box='./data/plate_VBR/BigBox_Lloyd.mat';
...
if ~exist(Files.SV_Box,'file') || recalc_SV==1
    ...
    LABsweep.TpC = 700:25:1800; %% HL has expanded this sweep
    % LABsweep.zPlatekm = 60:20:160; %% HL has expanded this sweep
    LABsweep.zPlatekm = 50:10:250;
...
Files.VBR_Box='./data/plate_VBR/thermalEvolution_Lloyd.mat';
VBRSettings.freq = logspace(-3,2.0,40);
...
%LAB_settings.per_bw_max = 30;  % max period to use for fitting (s) %% orig
%LAB_settings.per_bw_min = 10;  % min period to use for fitting (s)
LAB_settings.per_bw_max = 5.0;  % max period to use for fitting (s) %% Lloyd
LAB_settings.per_bw_min = 0.5;  % min period to use for fitting (s) %% Lloyd
LAB_settings.q_method = q_method; % match Q method above
%%%% CHANGE THIS FUNCTION TO DIFFERENT FUNCTION
%predicted_vals = calc_LAB_Vs(Files.VBR_Box, LAB_settings);
%%%% CHANGE TO THE FOLLOWING FUNCTION
predicted_vals = calc_LAB_AN(Files.VBR_Box, LAB_settings,fobs);
...
plot_plates_HL(posterior, q_method,outdir)
