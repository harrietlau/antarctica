function predicted_vals = calc_LAB_eta(VbrBoxFile,settings,fobs)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
function settings = checkFittingSettings(settings,VBRbox)
  ...
  Defaults.per_bw_max=0.9;  %% for Lloyd's model
  Defaults.per_bw_min=1.1;  %% for Lloyd's model
...
%     meanQs(iBox)=mean(Qs_zf_mask(:)); % ** Currently unused **
    ...
    Z_LAB_eta(iBox) = find_LAB_eta(Box(iBox),freq,ifreq);
     
  end
  
  
  predictions.zLAB_Q=Z_LAB_eta;
  predictions.meanVs=meanVs;
  
end

function Z_LAB_eta = find_LAB_eta(Box_data,freq,ifreq)

    eta = Box_data.out.viscous.HK2003.diff.eta;
    Gu = Box_data.out.elastic.Gu_0;
    z = Box_data.Z_km;

    maxf = 1./(eta./Gu);
    [~,idx]=min(abs(maxf-freq(ifreq)));
    Z_LAB_eta = z(idx);
