function predicted_vals = calc_LAB_NVG(VbrBoxFile,settings,fobs)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%   meanQs= zeros(size(Box)); % ** Currently unused **
  ...
  for iBox = 1:numel(Box)
    ...
    [ Vs_zf, ~, zi ] = interp_FreqZ(Vs_fz,freq,nn_pts,...
                                         Z_km,nn_pts);
                                                         
    %% HL turned off:
    %Z_LAB_eta(iBox) = find_LAB_Q(Qs_z,Z_km_interp,'method',q_LAB_method, ...
    %                                    'value',q_LAB_value,'z_min_km',zplate);
    Z_LAB_NVG(iBox) = find_LAB_NVG(zi,Vs_zf);
     
  end
  
  
  predictions.zLAB_Q=Z_LAB_NVG;
  predictions.meanVs=meanVs;
  
end

function Z_LAB_NVG = find_LAB_NVG(Z_km,Vs_zf)
    % find most negative velocity gradient
    nz = length(Z_km)-1;
    nf = length(Vs_zf(1,:));
    dvdz = zeros(nz-1,nf);
    z = zeros(nz,1);
    for i=1:nz
        for j=1:nf
            dvdz(i,j) = (Vs_zf(i+1,j)-Vs_zf(i,j))/ (Z_km(i+1)-Z_km(i));
        end
        z(i) = 0.5*(Z_km(i+1)+Z_km(i));
    end
    zlabs = zeros(nz,1);
    imin = find(z>45.);
    imin = imin(1);
    for i=1:nf
        idx = find(dvdz(:,i)==min(dvdz(imin:nz,i)));
        zlabs(i) = z(idx(1));
    end
    Z_LAB_NVG = mean(zlabs);
end
